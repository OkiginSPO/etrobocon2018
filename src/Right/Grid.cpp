...
static float GRID_X = 200; // 開発部の床が50cm  225.0; // 横(45cm) その中間点をとって22.5
static float GRID_Y = 200; // 開発部の床が50cm   200.0; // 縦(40cm) その中間点をとって20.0
...
void Grid::setDirection(int aX, int aY, int bX, int bY) {
    ...
    grid_direction = CalcDirection(bX, bY, aX, aY);
    //    grid_direction = ;
}
...
float Grid::CalcDirection(int aX, int aY, int bX, int bY) {
    ...
    //　座標aから座標bへの方位（ラジアン）を取得
    targetDir = atan2((float) (bX - aX), (float) (bY - aY));
